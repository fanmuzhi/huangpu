...
Ts_FlooredPixelsTest::Ts_FlooredPixelsTest(string &strName, string &strArgs, FpAlphaModule * &pDutCtrl, Syn_Dut * &pDut)
...
void Ts_FlooredPixelsTest::ProcessData()
{
	...
	if (!(_pSyn_Dut->_pSyn_DutTestResult->_flooredPixelsResults.m_bPass))
	{
		...
		_pSyn_Dut->_pSyn_DutTestResult->_mapTestPassInfo.insert(std::map<std::string, std::string>::value_type("FlooredPixelsTest", "Fail"));
	}
	else
		_pSyn_Dut->_pSyn_DutTestResult->_mapTestPassInfo.insert(std::map<std::string, std::string>::value_type("FlooredPixelsTest", "Pass"));
