...
extern "C" __declspec(dllexport) float __stdcall testSNRUG(unsigned char* pWiteimg,unsigned char* pSpoofimg,int iWidth,int iHeight,float *signalout,float* noiseout,int iNumofimg);
...
extern "C" __declspec(dllexport) int __stdcall CalTilt(unsigned char* pImage,int width ,int height ,int *whole,int *block);
extern "C" __declspec(dllexport) int __stdcall getAlgoVersion();