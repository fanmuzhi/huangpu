...
#define	BS0_SIZE			64
#define	BS1_SIZE			64
#define	MS0_SIZE			4 * 1024 //Size of MS0 and MS1.
...
typedef struct
{
	...
	float limit_overall_snr_high;
	float limit_overall_snr_low;
	float limit_overall_signal_high;
	float limit_overall_signal_low;
	float limit_overall_noise_high;
	float limit_overall_noise_low;
	float limit_zone_snr_high;
	float limit_zone_snr_low;
	float limit_zone_signal_high;
	float limit_zone_signal_low;
	float limit_zone_noise_high;
	float limit_zone_noise_low;
}SNRInfo;
...
typedef struct
{
	...
	float		m_nVCC;
	uint8_t		m_arWofData[MAX_WOF_DATA];
	uint8_t		m_arStartStopThreshGains[MAX_THRESHOLD_GAINS];
}TopSCM_WofTestInfo;
...
//////////////////////////// ////////////////////////
///////////////////////////	////////////////////////
//FUNCTION PROTOTYPES FOR SNR TEST
SNRInfo get_SNRinfo();
...
float sum(int arr[HISTOGRAM],int firstindex,int lastindex);
float get_signal_value(FPSFrame* fingerdata, FPSFrame* nofingerdata, SNRInfo* info, int minrow, int maxrow, int mincol, int maxcol,int regions, SNRResults* results);
...
//////////////////////////// ////////////////////////
///////////////////////////	////////////////////////
//FUNCTION PROTOTYPES for Delta Pixel Test
void SYN_DeltaPixelExecute(FPSMatrix* pNormalizedAvgs, DeltaPixelInfo* info, DeltaPixelResults* results,int nTrimLeft, int nTrimRight, int nTrimTop, int nTrimBottom,int numRows, int numCols);
int max_array(int a[], int num_elements);
// END OF FUNCTION PROTOTYPES for Delta Pixel Test

//FUNCTION PROTOTYPES for WofTest
void SYN_WofTestExecute(WofTestInfo* pInfo, WofTestResults* pResults);
void SYN_BottomSCMWofTestExecute(BottomSCM_WofTestInfo* pInfo, BottomSCM_WofTestResults* pResults);
void SYN_TopSCMWofTestExecute(TopSCM_WofTestInfo* pInfo, TopSCM_WofTestResults* pResults);
...
//FUNCTION PROTOTYPES for Pixel Patch Test
void SYN_PixelPatchTestExecute(PixelPatchInfo* pInfo, PixelPatchResults *pResults);

//FUNCTION PROTOTYPES for SPI Flash Patch Test
...
void get_otp_write_arguments(char* outString, int* numBytes, char* pVal2Write, char* m_printfile, int isCalibrate, int ROW );
...
void BS1_arguments_to_write(char* arData, int* numBytes,char* serial_number, char* bs1);
int check_if_written_BS0(unsigned char* arData, int numBytes);
void BS0_arguments_to_write(char* arData, char* bs0);
char second_calibration(float* avg);
int check_condition_temp(float* avg);
//////////////////////////////////////////////////////////////

//Image Tests
void SYN_SharpnessTest(int height, int width, float pImg[MAXROW][MAXCOL], float* sharpnessMeasure, SharpnessInfo* pInfo, SharpnessResults* pSharpness);
...
void permute(int numRowsImg, int numColsImg, int pImg[MAXROW][MAXCOL], int* numRowsArranged, int* numColsArranged, int pArragend[MAXROW][MAXCOL], int numOrder);
